...
def classify_action(keypoints, prev_keypoints=None):
    ...
    # Detect "Sleep" - When nose, shoulders, and hips are at nearly the same height (lying down)
    sleep_threshold = 30  # Max difference in height for "lying down" condition
    if (abs(nose_height - shoulder_height) < sleep_threshold and
        abs(shoulder_height - hip_height) < sleep_threshold):
        return "Sleep"

    # Detect "Clap"
    clap_threshold = 10  # Distance threshold for hands to be considered a clap
    min_movement = 5  # Minimum movement of hands towards each other
    ...
    # Detect "Wave" when either hand is above the nose
...
while cap.isOpened():
    ...
    # Show both windows
    cv2.imshow('Live Video', original_frame)  # Unaltered webcam feed
